...
#include "types.h"    // CSC matrix type
     ...
     */
    csc *
    csc_matrix(c_int m, c_int n, c_int nzmax, c_float *x, c_int *i, c_int *p);
     ...
     */
    csc *
    csc_spalloc(c_int m, c_int n, c_int nzmax, c_int values, c_int triplet);
     ...
     */
    csc *
    csc_done(csc *C, void *w, void *x, c_int ok);
     ...
     */
    csc *
    copy_csc_mat(const csc *A);

    /**
     *  Copy sparse CSC matrix A to B (B is preallocated, NO MALOC)
     */
    void
    prea_copy_csc_mat(const csc *A, csc *B);
     ...
     */
    csc *
    triplet_to_csc(const csc *T, c_int *TtoC);
     ...
     */
    csc *
    triplet_to_csr(const csc *T, c_int *TtoC);
     ...
     */
    c_int
    csc_cumsum(c_int *p, c_int *c, c_int n);
     ...
     */
    c_int *
    csc_pinv(c_int const *p, c_int n);
     ...
     */
    csc *
    csc_symperm(const csc *A, const c_int *pinv, c_int *AtoC, c_int values);
